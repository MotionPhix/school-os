...
- **Track 2 — PHPStan baseline reduction** ⏸ **paused after toolchain upgrade (046b65a, 5e227cb)**: Larastan 3.9.3 → **3.10.0**, PHPStan 2.1.45 → **2.2.10** (supports the Laravel 13 attribute syntax / v13 sample app); baseline regenerated **1129 → 1036 entries**. Also fixed **critical RCE advisory CVE-2026-44262** in `dedoc/scramble` (0.13.17 → 0.13.42). Typed 15 files: `HasUuid` trait (typed `creating` closure + dropped the dead `method_exists(Str::uuid7)` guard), `SystemRolesSeeder` (same guard), and relation generics (`BelongsTo<X, $this>` etc.) + `@property-read` + computed-column properties on 12 models (Student, Guardian, Campus, Invoice, Application, CourseSection, AttendanceSession, Exam, GradebookEntry, StaffMember, CalendarEvent, Term, Subject). Remaining ~1036 errors are genuine typing gaps (withCount/withSum aliases, mixed casts, untyped collections) — resume here any time.
- **Track 3 — Finance** ✅: row-lock races closed on the remaining finance verbs — `IssueInvoice`, `VoidInvoice`, `RefundPayment` now `lockForUpdate` the row and re-check state inside the transaction (payment path was already locked; void/refund are now too). Money-edge invariants verified: discount ≥ subtotal → issue rejected / totals clamped to 0, overdue transition on recompute, partial-payment refund restores balance with a mirror reversal, gateway-fee refund reverses all 3 postings balanced, payments rejected on voided invoices. 10 tests (`Finance/HardeningTest.php`) — suite 224 green. Note: no tax model exists in the domain — tax is a feature, not a hardening gap (flagged, not invented).
- **Track 4 — Attendance + assessments** ✅: attendance `summary` risk filter switched from float division to **exact integer math** (at_risk ⇔ (p+l)·10 < 9·sessions; critical ⇔ < 8·; perfect ⇔ ≥ 10·) so boundary rates (exactly 90% / 80% / 100%) can't drift; late counts as present-like. Report-card band boundaries pinned at every threshold (75/65/55/45/40), per-subject averaging + best-band, draft/marking exams excluded, zero-max-score guard, completeness (missing results don't zero the average). Rollover guards pinned: date-overlap conflicts, open-term block on close, supervised reopen, single-current-year, closed-year setCurrent rejection. 20 tests (RiskBandTest, ReportCardHardeningTest, RolloverTest) — suite 244 green.
- **Track 5 — Communications resilience** ✅ (115bc32): failure taxonomy (`BroadcastDeliveryFailureReason` — offline/connection_failed/unauthorized/timeout/rejected) recorded per-broadcast as an integer-distributed `failure_reasons` JSON (exact sum, remainder to largest bucket); `schoolos:retry-broadcast-deliveries` every 15 min retries failed deliveries with exponential backoff (base 15 min × 2^attempt, capped 240 min, all env-tunable) and **dead-letters** broadcasts past `max_retries` (3); `CheckBroadcastDeliveries` now alerts **only dead-lettered/retry-exhausted** broadcasts (in-flight retries aren't "failures") and the alert payload carries retry_count, dead_lettered and the reason breakdown. 6 tests (`DeliveryResilienceTest`) + 2 ObservabilityTest fixtures updated to the new contract — suite 250 green.
- **Live E2E — auth → console flow validated ✅ + 1 real bug fixed (d9b1e6f)**: ran the complete flow against the live ngrok backend — register (201, token) → `/me` 403 (unverified = verify-screen signal) → email verified via the **real signed link from the mail log** (204) → `/me` shows `email_verified=true, has_memberships=false` (onboarding) → create tenant (201, creator = principal, member_count=1) → `/me` shows `has_memberships=true, roles=principal, 63 permission keys` (console) → logout (204) → re-login (201) → console state persists. Negatives: wrong password 422, duplicate email 422, second tenant 403 (day-0 policy), re-verify 204 (idempotent). **Bug found & fixed**: unauthenticated API requests without `Accept: application/json` hit Laravel's `Authenticate` default redirect → `route('login')` (does not exist) → masked 500. Fix: `redirectGuestsTo(null for api/*)` + `shouldRenderJsonWhen(api/*)` in `bootstrap/app.php`; regression test added (suite 251). Note: running server needs a restart to pick up the fix.
- **Frontend — schoolos-ui — live API wiring** ✅ (no git repo on the frontend; changes are on disk): `.env.local` → `VITE_API_MODE=live`, `VITE_API_URL=https://disabled-subpar-overtime.ngrok-free.dev`. Transport overrides added for the non-conventional auth routes (session.login/register/me/logout/switch_tenant/verify_email). `session.tsx` is now live-aware: token-based hydration refreshing `/me` (landing contract authoritative), async signIn/signUp/verifyEmail/createOwnerTenant/signOut/switchTenant, `needsOnboarding` derived from `has_memberships` (the API has no `onboarding_completed_at`), and the tenant list synced from `/identity/tenants`. login/signup/verify/onboarding updated to pass password / await / accept the real signed-link `id`+`hash` params. Verified: `tsc` clean on all touched files (11 pre-existing stale-router-type errors in untouched files), tunnel reachable, CORS `*` confirmed, login endpoint responds 422 on bad creds. Backend needed no changes (CORS was already open). UI enhancement pass (live data) — first milestone ✅: verify screen rebuilt for live mode — paste-link field (accepts the full signed URL or `id:hash`), **Resend verification email** button (wired to `identity.session.resend_verification`, anti-enumeration confirmed server-side; correctly no-ops for already-verified accounts), dev hint pointing at `storage/logs/laravel.log` (log mailer). Console data layer audited: all list screens read via `dispatch("…list")` verbs that resolve to real endpoints (students/campuses/invoices/profile all 200 against live; empty arrays for the fresh tenant). Pushed as `198d736`. **Onboarding hardening #2 (frontend `998195e`)**: (1) the live-mode guard no longer yanks the owner out of the wizard — `needsOnboarding` is `!has_memberships || onboardingPending` with a localStorage owner flag set at tenant creation and cleared at finish (invited members never run the wizard → console directly); (2) the **academic year is now a required step** (was labelled Optional + skippable); (3) the campus step now requires region + street address, matching `StoreCampusRequest` (previously silent 422s at review). Known gap: the invite step sends mock role ids (`rol_teacher`) — live invites need the tenant's real role ids (`GET /identity/roles`) before they can succeed. Next: seed the E2E tenant via the UI or API (campus → students → subjects…) to exercise the screens with real data.
