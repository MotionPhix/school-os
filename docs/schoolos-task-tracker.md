...
4. **Realtime — ✅ slice 1+2 DONE (2026-08-30)**: Reverb transport; `BroadcastServiceProvider` (`/broadcasting/auth` on sanctum+force.json+resolve.tenant); channels from `App\Support\RealtimeChannels` (private `users.{id}`/`tenant.{id}` + presence `sessions.{id}`/`exams.{id}`/`threads.{id}` with permission/participant gates); **feed badge push** (`FeedBadgeUpdated`), **broadcast progress ticks** (`BroadcastProgressUpdated`), **live register counts** (`SessionMarksUpdated` on the session presence), **thread reply push** (`ThreadReplyCreated`). Recording-broadcaster tests (11). Remaining: timetable conflict push, tenant-wide fan-out — suite 184 green.
