...
4. **Realtime — 🟡 slice 1 DONE (2026-08-30)**: Reverb installed (transport), `BroadcastServiceProvider` (`/broadcasting/auth` on sanctum+force.json), channels from `App\Support\RealtimeChannels` (`private-users.{id}`, `tenant.{id}`), **feed badge push** (`FeedBadgeUpdated`, unread count, emitted from `TenantDatabaseChannel`) + **broadcast progress ticks** (`BroadcastProgressUpdated`, start/complete counters). Recording-broadcaster tests (6). Remaining: marksheet/register presence, thread reply presence, tenant-wide fan-out — suite 179 green.
